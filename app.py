...
if submitted:
    if not current_place_name or not target_area or not main_menu:
        ...
        with st.spinner("AI가 네이버 도구 가산점 누락 여부와 상권 데이터를 분석 중입니다..."):
            ...
            # 도구 세팅 현황 텍스트화 ('미사용' -> '미등록'으로 수정)
            tool_status = f"예약({'사용' if use_booking else '미등록'}), 톡톡({'사용' if use_talktalk else '미등록'}), 쿠폰({'사용' if use_coupon else '미등록'}), 안심번호({'사용' if use_safecall else '미등록'})"
            ...
            ###PROBLEM###
            (현재 도구 세팅 현황({tool_status})을 짚어주면서, 네이버 공식 도구를 제대로 등록하지 않아서 알고리즘 가산점을 못 받고 있으며 이로 인해 경쟁 매장들에 비해 순위가 계속 밀리고 있다는 팩트를 뼈 때리게 진단해 1~2줄)

            ###EFFECT###
            (미등록 상태인 도구를 전부 세팅하고 최적화했을 때, 네이버 알고리즘 체류시간 가산점을 획득하여 노출 순위가 회복되고 고객 방문 전환율이 얼마나 상승할지 기대 효과를 1~2줄로 작성해)
